...
describe('IRReceive', function() {
  ...
  afterEach(function() {
    ...
    lirc_node.IRReceive._setInputListener(
      lirc_node.IRReceive._DEFAULT_INPUT_LISTENER
    )
  })
  ...
  describe('#addListener', function() {
    ...
    it('should spawn a new child process to listen for input, if it is not already running', function() {
      assert.equal(
        undefined,
        lirc_node.IRReceive._getListeningProcess(),
    ...
    it('should throw if given unexpected argument types', function() {
      ...
      assert.doesNotThrow(
        irreceive.addListener.bind(irreceive, 'KEY_OK', function() {})
      )
      assert.doesNotThrow(
        irreceive.addListener.bind(
          irreceive,
          'KEY_OK',
          'my_remote',
          function() {}
      ...
      assert.doesNotThrow(
        irreceive.addListener.bind(irreceive, 'KEY_OK', function() {}, 1000)
      )
      assert.doesNotThrow(
        irreceive.addListener.bind(irreceive, function() {}, 1000)
      )
      ...
      assert.doesNotThrow(
        irreceive.addListener.bind(irreceive, function() {}, 1000, 'one', 'two')
      )
      ...
      assert.throws(
        irreceive.addListener.bind(irreceive, 'string but no callback function')
      )
      assert.throws(
        irreceive.addListener.bind(
          irreceive,
          'string but no callback function',
          'and another'
        )
      )
      assert.throws(
      ...
      assert.throws(
        irreceive.addListener.bind(irreceive, 'one', function() {}, 'two', 1000)
      )
      assert.throws(
        irreceive.addListener.bind(irreceive, function() {}, 'one', 'two', 1000)
      )
      assert.throws(
        irreceive.addListener.bind(irreceive, function() {}, 'one', 1000)
      )
      assert.throws(irreceive.addListener.bind(irreceive, 1))
      assert.throws(irreceive.addListener.bind(irreceive, 1000, function() {}))
      assert.throws(
        irreceive.addListener.bind(irreceive, 1000, 'one', function() {})
      )
      assert.throws(
        irreceive.addListener.bind(irreceive, 1000, 'one', 'two', function() {})
      )
      assert.throws(
        irreceive.addListener.bind(irreceive, 1000, 'one', function() {}, 'two')
      )
      assert.throws(
        irreceive.addListener.bind(irreceive, 1000, function() {}, 'one', 'two')
      )
    })
  })
